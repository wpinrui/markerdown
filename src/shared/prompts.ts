...
// Markers to identify agent use cases in session history (shown in chat history dropdown)
export const AGENT_MARKERS = {
  CHAT: '[CHAT]',
  SUMMARIZE: '[SUMMARIZE]',
  // Add more markers here as new use cases are added
}

export function getSummarizePrompt(pdfPath: string, outputPath: string, userPrompt: string, memoryContext: string): string {
  return `${AGENT_MARKERS.SUMMARIZE}
${memoryContext}Read the PDF at "${pdfPath}". Then create a markdown file at "${outputPath}" with the following:

${userPrompt}`
}

export function getAgentSystemPrompt(memoryContext: string): string {
  return `${AGENT_MARKERS.CHAT}
You are a helpful assistant that answers questions about the files in this directory.
